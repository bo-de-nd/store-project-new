...
const parse = (row) => row ? { ...row, sizes: row.sizes || [], images: row.images || [] } : null;
...
router.post("/", requireAdmin, async (req, res) => {
  try {
    ...
    const r = await query(
      ...
      [productId, name, category, price, color || "", JSON.stringify(sizes || []), stock || 0, JSON.stringify(images || [])]
    );
...
router.put("/:id", requireAdmin, async (req, res) => {
  try {
    ...
    const r = await query(
      `UPDATE products SET name=$1,category=$2,price=$3,color=$4,sizes=$5,stock=$6,images=$7 WHERE id=$8 RETURNING *`,
      [
        name ?? p.name, category ?? p.category, price ?? p.price, color ?? p.color,
        JSON.stringify(sizes ?? p.sizes), stock ?? p.stock,
        JSON.stringify(images ?? p.images), req.params.id
      ]
    );
    res.json(parse(r.rows[0]));
  } catch (e) { res.status(500).json({ error: e.message }); }
